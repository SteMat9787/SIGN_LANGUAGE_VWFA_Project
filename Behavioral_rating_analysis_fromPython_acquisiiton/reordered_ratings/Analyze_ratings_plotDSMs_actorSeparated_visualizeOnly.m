...
for vr=1:length(combos)
    respVis_reord(vr,1)=respVis(actual_pos(vr));
end % for vr (visual ratings)

for sr=1:length(combos)
    respSem_reord(sr,1)=respSem(actual_pos(sr));
...
respVis_reord=1-respVis_reord;
respVis_reord=mat2gray(respVis_reord);
Vis_DSM=squareform(respVis_reord);
...
title (strcat( 'Visual Ratings-',sub_ID));
...
respSem_reord=1-respSem_reord;
respSem_reord=mat2gray(respSem_reord);
Sem_DSM=squareform(respSem_reord);
Sem_DSM_tril=tril(Sem_DSM);
imagesc(Sem_DSM_tril);
colorbar;
set(colorbar,'FontName','Avenir','FontSize',18, 'FontWeight','bold',...
       'LineWidth',2.5,'TickDir','out', 'TickLength', [0]);
title (strcat( 'Semantic Ratings-',sub_ID));
set(gca, 'YTick',(1:length(labels)),'YTickLabel',labels);
set(gca, 'XTick',(1:length(labels)),'XTickLabel',labels);
xtickangle(45);
set(gca,'FontName','Avenir','FontSize',18, 'FontWeight','bold',...
