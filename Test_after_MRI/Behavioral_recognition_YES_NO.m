...
% %     [wPtr, rect]= Screen('OpenWindow',max(Screen('Screens'))); %open the screen
    ...
    DrawFormattedText(wPtr, '\n  IN THIS EXPERIMENT YOU WILL SEE SOME VIDEOS OF SIGNS. SOME ARE REAL SIGNS FROM BELGIAN SIGN LANGUAGE, OTHERS ARE FALSE MEANINGLESS SIGNS (press ENTER to continue)', 'center','center',[255 255 255],60,[],[],3);
...
%     [possible_pairs]=Create_pairs(All_stim);
    ...
    % Instruction
    DrawFormattedText(wPtr, '\n   At the end of each sign you will have to say if you think the sign was meaningful or not (press ENTER to continue)', 'center','center',[255 255 255],60,[],[],3);
    ...
    DrawFormattedText(wPtr, '\n    Press "y" for YES and "n" for NO (press ENTER to continue)', 'center','center',[255 255 255],60,[],[],3);
    Screen('Flip', wPtr); %display the info visually
    KbWait( -1, 2 ); %wait for key press
...
elseif size==2 %modified size
    ...
    Screen('TextSize', wPtr, 40);
    DrawFormattedText(wPtr, '\n  DO YOU THINK THIS SIGN WAS REAL? (Y for YES / N for NO)', 'center','center',[255 255 255],60,[],[],3);
    Screen('Flip', wPtr);
